...
TAX_AMT	float                            ,
CURR_STS_ID	INT                          ,
BILL_TO_ADDR_ID	varchar(50)              ,
BILL_TO_CUST_ID	varchar(50)              ,
CUST_CNTCT_ID	varchar(50)              ,
GUEST_ORDER_IND	int                      ,
CORP_ORDER_IND	int                      ,
DEVICE_TYPE	varchar(50)                  ,
IP_ADDR	varchar(50)                      ,
FRAUD_CHECK_REQD_IND	int              ,
ORIG_SALES_ORDER_NUM	varchar(50)      ,
HOLD_FLAG	int                          ,
XCHNG_TYPE_ID	INT                      ,
ORDER_TYPE_ID	INT                      ,
WM_STORE_ORG_ID	INT                      ,
...
CREATE TABLE FACT_sales_order_line_IN1542  
(                               
								
SALES_ORDER_NUM	BIGINT          ,
SALES_ORDER_LINE_NUM	INT     ,
TENANT_ORG_ID	INT             ,
SHIP_NODE_ORG_ID	varchar(50) ,
GIFT_IND	int                 ,
ORDER_PLACED_DT	DATE            ,
ORDER_PLACED_TS	DATETIME        ,
SVC_NM	varchar(50)             ,
...
QTY	float                       ,
SHPD_QTY	varchar(50)         ,
SHIP_TO_ADDR_ID	varchar(50)     ,
WM_ITEM_NUM	varchar(50)         ,
UPC	varchar(50)                 ,
CATLG_ITEM_ID	varchar(50)     ,
PROD_OFFR_ID	varchar(50)     ,
RQ_CANCEL_TS	DATETIME        ,
RQ_SHIP_TS	DATETIME            ,
SLR_ORG_ID	INT                 ,
SHIP_TO_STORE_ORG_ID	INT     ,
ORDER_STS_ID	INT             ,
PROD_CLASS_ID	varchar(50)     ,
MAX_DAYS_TO_SHIP	varchar(50) ,
EARLIEST_DLVR_TS	DATETIME    ,
ORIG_EXPC_DLVR_TS	DATETIME    ,
CARRIER_EXPC_DLVR_TS	DATETIME,
PROMO_ID	varchar(50)         ,
FULFMT_TYPE_ID	varchar(50)     ,
ORDER_LINE_TYPE_ID	INT         ,
LINE_TOT_AMT	float           ,
ORIG_SALES_ORDER_LINE_NUM	INT ,
PERISHABLE_IND	int             ,
ITEM_CLASS_ID	INT             ,
CATLG_ITEM_KEY	INT             ,
PROD_OFFR_KEY	INT             ,
RQ_CANCEL_TS_KEY	INT         ,
RQ_SHIP_TS_KEY	INT             ,
ORDER_STS_KEY	INT             ,
EARLIEST_DLVR_TS_KEY	INT     ,
ORIG_EXPC_DLVR_TS_KEY	INT     ,
CARRIER_EXPC_DLVR_TS_KEY	INT ,
SLR_ORG_KEY	INT                 ,
SHIP_TO_STORE_KEY	INT)   
---------------------------------------------------------------------------------------------
...
CHARGE_CATEG_ID	INT
CHRG_CATEG_MAP_ID	INT
CHARGE_NM	VARCHAR(50)
CHARGE_AMT	DECIMAL(19,6)
CHRG_CRE_DT	DATETIME
CHRG_QTY	INT
CHRG_CRE_DT_KEY	INT
charge_categ_key	INT)
------------------------------------------------------------------------------------------
CREATE TABLE FACT_RTN_ORDER_LINE_IN1542(
...
-----------ROW COUNT-------------
SELECT COUNT(*) FROM FACT_ORDER_LINE_CHRG_IN1542 
SELECT COUNT(*) FROM (
 select 
...
----------------ROW COUNT GROUP BY---------------------
SELECT * FROM FACT_ORDER_LINE_CHRG_IN1542 
...
SELECT  COUNT(*) FROM FACT_ORDER_LINE_CHRG_IN1542 T LEFT JOIN
  (
 select 
...
ON T.SALES_ORDER_NUM  =S.SALES_ORDER_NUM 
WHERE S.SALES_ORDER_NUM  IS NULL
...
INSERT INTO FACT_sales_order_line_IN1542 

SELECT(
--,IIF(SALES_ORDER_NUM    IS NULL OR   SALES_ORDER_NUM='null',101,CONVERT(LTRIM(RTRIM(BIGINT,SALES_ORDER_NUM))))as SALES_ORDER_NUM
--,IIF(SALES_ORDER_LINE_NUM    IS NULL OR   SALES_ORDER_LINE_NUM='null',101,CONVERT(LTRIM(RTRIM(INT,SALES_ORDER_LINE_NUM))))as SALES_ORDER_LINE_NUM
--,IIF(TENANT_ORG_ID    IS NULL OR   TENANT_ORG_ID='null',101,CONVERT(LTRIM(RTRIM(INT,TENANT_ORG_ID))))as TENANT_ORG_ID
--,IIF(SHIP_NODE_ORG_ID    IS NULL OR   SHIP_NODE_ORG_ID='null','N/A',CONVERT(LTRIM(RTRIM(INT,ship_node_org_id))))as SHIP_NODE_ORG_ID
--,IIF(GIFT_IND    IS NULL OR   GIFT_IND='null',101,CONVERT(LTRIM(RTRIM(int,GIFT_IND))))as GIFT_IND
--,IIF(ORDER_PLACED_DT    IS NULL OR   ORDER_PLACED_DT='null','01-01-1900',CONVERT(LTRIM(RTRIM(DATE,ORDER_PLACED_DT))))as ORDER_PLACED_DT
--,IIF(ORDER_PLACED_TS    IS NULL OR   ORDER_PLACED_TS='null','01-01-1900',CONVERT(LTRIM(RTRIM(DATETIME,ORDER_PLACED_TS))))as ORDER_PLACED_TS
--,IIF(SVC_NM    IS NULL OR   SVC_NM='null','N/A',LTRIM(RTRIM(SVC_NM)))as SVC_NM
--,IIF(UNIT_PRICE    IS NULL OR   UNIT_PRICE='null', ,CONVERT(LTRIM(RTRIM(float,UNIT_PRICE))))as UNIT_PRICE
--,IIF(UNIT_COST    IS NULL OR   UNIT_COST='null', ,CONVERT(LTRIM(RTRIM(float,UNIT_COST))))as UNIT_COST
--,IIF(QTY    IS NULL OR   QTY='null', ,CONVERT(LTRIM(RTRIM(float,QTY))))as QTY
--,IIF(SHPD_QTY    IS NULL OR   SHPD_QTY='null','N/A',CONVERT(LTRIM(RTRIM(float,SHPD_QTY))))as SHPD_QTY
--,IIF(SHIP_TO_ADDR_ID    IS NULL OR   SHIP_TO_ADDR_ID='null','N/A',LTRIM(RTRIM(SHIP_TO_ADDR_ID)))as SHIP_TO_ADDR_ID
--,IIF(WM_ITEM_NUM    IS NULL OR   WM_ITEM_NUM='null','N/A',LTRIM(RTRIM(WM_ITEM_NUM)))as WM_ITEM_NUM
--,IIF(UPC    IS NULL OR   UPC='null','N/A',LTRIM(RTRIM(UPC)))as UPC
--,IIF(CATLG_ITEM_ID    IS NULL OR   CATLG_ITEM_ID='null','N/A',LTRIM(RTRIM(CATLG_ITEM_ID)))as CATLG_ITEM_ID
--,IIF(PROD_OFFR_ID    IS NULL OR   PROD_OFFR_ID='null','N/A',LTRIM(RTRIM(PROD_OFFR_ID)))as PROD_OFFR_ID
--,IIF(RQ_CANCEL_TS    IS NULL OR   RQ_CANCEL_TS='null','01-01-1900',CONVERT(LTRIM(RTRIM(DATETIME,RQ_CANCEL_TS))))as RQ_CANCEL_TS
--,IIF(RQ_SHIP_TS    IS NULL OR   RQ_SHIP_TS='null','01-01-1900',CONVERT(LTRIM(RTRIM(DATETIME,RQ_SHIP_TS))))as RQ_SHIP_TS
--,IIF(SLR_ORG_ID    IS NULL OR   SLR_ORG_ID='null',101,CONVERT(LTRIM(RTRIM(INT,SLR_ORG_ID))))as SLR_ORG_ID
--,IIF(SHIP_TO_STORE_ORG_ID    IS NULL OR   SHIP_TO_STORE_ORG_ID='null',101,CONVERT(LTRIM(RTRIM(INT,SHIP_TO_STORE_ORG_ID))))as SHIP_TO_STORE_ORG_ID
--,IIF(ORDER_STS_ID    IS NULL OR   ORDER_STS_ID='null',101,CONVERT(LTRIM(RTRIM(INT,ORDER_STS_ID))))as ORDER_STS_ID
--,IIF(PROD_CLASS_ID    IS NULL OR   PROD_CLASS_ID='null','N/A',LTRIM(RTRIM(PROD_CLASS_ID)))as PROD_CLASS_ID
--,IIF(MAX_DAYS_TO_SHIP    IS NULL OR   MAX_DAYS_TO_SHIP='null','N/A',LTRIM(RTRIM(MAX_DAYS_TO_SHIP)))as MAX_DAYS_TO_SHIP
--,IIF(EARLIEST_DLVR_TS    IS NULL OR   EARLIEST_DLVR_TS='null','01-01-1900',CONVERT(LTRIM(RTRIM(DATETIME,EARLIEST_DLVR_TS))))as EARLIEST_DLVR_TS
--,IIF(ORIG_EXPC_DLVR_TS    IS NULL OR   ORIG_EXPC_DLVR_TS='null','01-01-1900',CONVERT(LTRIM(RTRIM(DATETIME,ORIG_EXPC_DLVR_TS))))as ORIG_EXPC_DLVR_TS
--,IIF(CARRIER_EXPC_DLVR_TS    IS NULL OR   CARRIER_EXPC_DLVR_TS='null','01-01-1900',CONVERT(LTRIM(RTRIM(DATETIME,CARRIER_EXPC_DLVR_TS))))as CARRIER_EXPC_DLVR_TS
--,IIF(PROMO_ID    IS NULL OR   PROMO_ID='null','N/A',LTRIM(RTRIM(PROMO_ID)))as PROMO_ID
--,IIF(FULFMT_TYPE_ID    IS NULL OR   FULFMT_TYPE_ID='null','N/A',LTRIM(RTRIM(FULFMT_TYPE_ID)))as FULFMT_TYPE_ID
--,IIF(ORDER_LINE_TYPE_ID    IS NULL OR   ORDER_LINE_TYPE_ID='null',101,CONVERT(LTRIM(RTRIM(INT,ORDER_LINE_TYPE_ID))))as ORDER_LINE_TYPE_ID
--,IIF(LINE_TOT_AMT    IS NULL OR   LINE_TOT_AMT='null', ,CONVERT(LTRIM(RTRIM(float,LINE_TOT_AMT))))as LINE_TOT_AMT
--,IIF(ORIG_SALES_ORDER_LINE_NUM    IS NULL OR   ORIG_SALES_ORDER_LINE_NUM='null',101,CONVERT(LTRIM(RTRIM(INT,ORIG_SALES_ORDER_LINE_NUM))))as ORIG_SALES_ORDER_LINE_NUM
--,IIF(PERISHABLE_IND    IS NULL OR   PERISHABLE_IND='null',101,CONVERT(LTRIM(RTRIM(int,PERISHABLE_IND))))as PERISHABLE_IND
--,IIF(ITEM_CLASS_ID    IS NULL OR   ITEM_CLASS_ID='null',101,CONVERT(LTRIM(RTRIM(INT,ITEM_CLASS_ID))))as ITEM_CLASS_ID
--,CATLG_ITEM_KEY   as CATLG_ITEM_KEY
--,PROD_OFFR_KEY  as PROD_OFFR_KEY
--,RQ_CANCEL_TS_KEY   as RQ_CANCEL_TS_KEY
--,RQ_SHIP_TS_KEY    as RQ_SHIP_TS_KEY
--,ORDER_STS_KEY   as ORDER_STS_KEY
--,EARLIEST_DLVR_TS_KEY  as EARLIEST_DLVR_TS_KEY
--,ORIG_EXPC_DLVR_TS_KEY   as ORIG_EXPC_DLVR_TS_KEY
--,CARRIER_EXPC_DLVR_TS_KEY   as CARRIER_EXPC_DLVR_TS_KEY
--,IIF(SLR_ORG_KEY    as SLR_ORG_KEY
--,SHIP_TO_STORE_KEY   as SHIP_TO_STORE_KEY

FROM 
[BCMPWMT].[SALES_ORDER_LINE]
----------------------------------------------------
...
SELECT COUNT(*) FROM fact_sales_order_IN1542 
SELECT COUNT(*) FROM 
(SELECT 
IIF(so.SALES_ORDER_NUM    IS NULL OR   so.SALES_ORDER_NUM='null',101,CONVERT(BIGINT,LTRIM(RTRIM(so.SALES_ORDER_NUM))))as SALES_ORDER_NUM
,IIF(so.TENANT_ORG_ID    IS NULL OR   so.TENANT_ORG_ID='null',101,CONVERT(INT,LTRIM(RTRIM(so.TENANT_ORG_ID))))as TENANT_ORG_ID
...
iif(so.ORDER_PLACED_TS is null or so.ORDER_PLACED_TS='null','01-01-1900',convert(datetime,so.ORDER_PLACED_TS))=DH1.date_id
AND iif(so.ORDER_PLACED_TS is null or so.ORDER_PLACED_TS='null',-1, DATEPART(HOUR,so.ORDER_PLACED_TS))=DH1.HOUR_ID
LEFT JOIN dim_day_SQL_IN1542 D1 ON iif(so.ORDER_PLACED_DT is null or so.ORDER_PLACED_DT='null','01-01-1900',convert(date,so.ORDER_PLACED_DT))= d1.DATE_ID)S

---------------------------ROW COUNT GROUP BY --------------
SELECT DEVICE_TYPE,COUNT(*)FROM fact_sales_order_IN1542 GROUP BY DEVICE_TYPE


SELECT DEVICE_TYPE, COUNT(*) FROM 
(SELECT 
IIF(so.SALES_ORDER_NUM    IS NULL OR   so.SALES_ORDER_NUM='null',101,CONVERT(BIGINT,LTRIM(RTRIM(so.SALES_ORDER_NUM))))as SALES_ORDER_NUM
,IIF(so.TENANT_ORG_ID    IS NULL OR   so.TENANT_ORG_ID='null',101,CONVERT(INT,LTRIM(RTRIM(so.TENANT_ORG_ID))))as TENANT_ORG_ID
,IIF(so.ORDER_REL_TS    IS NULL OR   so.ORDER_REL_TS='null','01-01-1900',CONVERT(DATETIME,LTRIM(RTRIM(so.ORDER_REL_TS))))as ORDER_REL_TS
...
SELECT 
count(SO.ORDER_REL_TS_KEY),
count(SO.ORDER_REL_DT_KEY),
count(SO.ORDER_PLACED_TS_KEY),
count(SO.ORDER_PLACED_DT_KEY ),
COUNT(SO.CURR_STS_ID_KEY),
COUNT(SO.BILL_TO_ADDR_ID_KEY),
COUNT(SO.BILL_TO_CUST_ID_KEY),
COUNT(SO.CUST_CNTCT_ID_KEY)

FROM 
fact_sales_order_IN1542 SO LEFT JOIN   DIM_STS_LKP_sql_IN1542  st  ON ST.STS_LKP_KEY=SO.CURR_STS_ID_KEY
...
-------------------------------------------------STARTIFICATION CHECK GROUP BY--------------------
SELECT * FROM fact_sales_order_IN1542 

SELECT SUM(TAX_AMT) TAX_AMT,SUM(TOT_AMT) TOT_AMT
FROM fact_sales_order_IN1542 

SELECT SUM(TAX_AMT),SUM(TOT_AMT)
FROM 
(SELECT 
IIF(so.SALES_ORDER_NUM    IS NULL OR   so.SALES_ORDER_NUM='null',101,CONVERT(BIGINT,LTRIM(RTRIM(so.SALES_ORDER_NUM))))as SALES_ORDER_NUM
,IIF(so.TENANT_ORG_ID    IS NULL OR   so.TENANT_ORG_ID='null',101,CONVERT(INT,LTRIM(RTRIM(so.TENANT_ORG_ID))))as TENANT_ORG_ID
...
SELECT SUM(TAX_AMT) TAX_AMT,SUM(TOT_AMT)TOT_AMT
FROM fact_sales_order_IN1542  GROUP BY DEVICE_TYPE
...
on iif(so.ORDER_REL_DT is null or so.ORDER_REL_DT='null','01-01-1900',convert(date,so.ORDER_REL_DT))
 d1.hour_id

so.ORDER_REL_TS  
SELECT * FROM [BCMPWMT].[SALES_ORDER]
  SELECT * FROM  dim_day_SQL_IN1542 D
 SELECT * FROM  DIM_DAY_HOUR_sql_IN1542 DH
    ,   as CURR_STS_ID_KEY
,   as BILL_TO_ADDR_ID_KEY
, as BILL_TO_CUST_ID_KEY
,   as CUST_CNTCT_ID_KEY

select * from DIM_DAY_HOUR_sql_IN1542 
select * from dim_day_SQL_IN1542
SELECT * FROM 
fact_sales_order_IN1542  
select * from [BCMPWMT].[SALES_ORDER]
SELECT * FROM  DIM_STS_LKP_sql_IN1542  
SELECT * FROM  dim_cust_address_SQL_IN1542 
SELECT * FROM  dim_cust_addr1_sql_IN1542
SELECT * FROM  DIM_CUST_CNTCT_SQL_IN1542
CURR_STS_ID= sts_ID
BILL_TO_ADDR_ID= addr_ID
BILL_TO_CUST_ID=addr1_id
CUST_CNTCT_ID=cntct_id

FROM [BCMPWMT].[SALES_ORDER] so left join DIM_DAY_HOUR_sql_IN1542 dh
	on CONVERT(DATE,F.ORDER_REL_TS)=DH.DATE_ID
   AND
   DH.hour_id	 = DATEPART(HOUR,F.ORDER_REL_TS)
 left join   DIM_STS_LKP_sql_IN1542  st  
 on IIF(so.CURR_STS_ID    IS NULL OR   so.CURR_STS_ID='null',101,CONVERT(INT,LTRIM(RTRIM(so.CURR_STS_ID))))=st.sts_id 
 left join dim_cust_address_SQL_IN1542 addr 
 on addr.addr_id = IIF(so.BILL_TO_ADDR_ID    IS NULL OR   so.BILL_TO_ADDR_ID='null','N/A',LTRIM(RTRIM(so.BILL_TO_ADDR_ID)))
 left join dim_cust_addr1_sql_IN1542 addr1 
 on IIF(so.BILL_TO_CUST_ID    IS NULL OR  so.BILL_TO_CUST_ID='null','N/A',LTRIM(RTRIM(so.BILL_TO_CUST_ID)))=addr1.addr_id  
 left join DIM_CUST_CNTCT_SQL_IN1542 c 
 on c.CNTCT_ID=IIF(so.CUST_CNTCT_ID    IS NULL OR   so.CUST_CNTCT_ID='null','N/A',LTRIM(RTRIM(so.CUST_CNTCT_ID)))

UPDATE fact_sales_order_IN1542  SET ORDER_REL_TS_KEY= DH.DAY_HOUr_key
   FROM fact_sales_order_IN1542 F JOIN DIM_DAY_HOUR_sql_IN1542 DH ON
   CONVERT(DATE,F.ORDER_REL_TS)=DH.DATE_ID
   AND
   DH.hour_id	 = DATEPART(HOUR,F.ORDER_REL_TS)
   --------------------------------------------------------   
   UPDATE fact_sales_order_IN1542 SET ORDER_PLACED_TS_KEY= DH.DAY_HOUr_key
   FROM fact_sales_order_IN1542 F JOIN DIM_DAY_HOUR_sql_IN1542  DH ON
   CONVERT(DATE,F.ORDER_PLACED_TS)=DH.DATE_ID
   AND
   DH.hour_id = DATEPART(HOUR,F.ORDER_PLACED_TS)
   --------------------------------------------------------
   UPDATE fact_sales_order_IN1542 SET [ORDER_REL_DT_KEY]= DH.DAY_KEY
   FROM fact_sales_order_IN1542 F JOIN dim_day_SQL_IN1542 DH ON
   CONVERT(DATE,F.[ORDER_REL_DT])=DH.DATE_ID
   ----------------------------------------------------------
   UPDATE fact_sales_order_IN1542 SET [ORDER_PLACED_DT_KEY]= DH.DAY_KEY
   FROM fact_sales_order_IN1542 F JOIN dim_day_SQL_IN1542 DH ON
...
"UPDATE FACT_SALES_ORDER SET ORDER_REL_TS_KEY= DH.DIM_DAY_HR 
   FROM FACT_SALES_ORDER F JOIN dim_day_hr DH ON
   CONVERT(DATE,F.ORDER_REL_TS)=DH.DATE_ID
   AND
   DH.HR_ID = DATEPART(HOUR,F.ORDER_REL_TS)
   --------------------------------------------------------   
   UPDATE FACT_SALES_ORDER SET ORDER_PLACED_TS_KEY= DH.DIM_DAY_HR 
   FROM FACT_SALES_ORDER F JOIN dim_day_hr DH ON
   CONVERT(DATE,F.ORDER_PLACED_TS)=DH.DATE_ID
   AND
   DH.HR_ID = DATEPART(HOUR,F.ORDER_PLACED_TS)
   --------------------------------------------------------
   UPDATE FACT_SALES_ORDER SET [ORDER_REL_DT_KEY]= DH.DAY_KEY
   FROM FACT_SALES_ORDER F JOIN dim_day DH ON
   CONVERT(DATE,F.[ORDER_REL_DT])=DH.DATE_ID
   ----------------------------------------------------------
   UPDATE FACT_SALES_ORDER SET [ORDER_PLACED_DT_KEY]= DH.DAY_KEY
   FROM FACT_SALES_ORDER F JOIN dim_day DH ON
   CONVERT(DATE,F.[ORDER_PLACED_DT])=DH.DATE_ID
"
...
select * from [BCMPWMT].[SALES_ORDER_ADJ]
 SELECT * FROM  DIM_DAY_HOUR_sql_IN1542 DH
select RSN_CD from [BCMPWMT].[SALES_ORDER_ADJ]
...
--------------ROW COUNT-------------1066
SELECT COUNT(*) FROM FACT_sales_order_adj_IN1542 
SELECT COUNT(*) FROM (
...
left join DIM_DAY_HOUR_sql_IN1542 DH2
on  DH2.DATE_ID=iif(oa.RFND_TS  IS NULL OR oa.RFND_TS  ='NULL','01-01-1900',CONVERT(DATE,oa.RFND_TS))
and DH2.hour_id=iif(oa.RFND_TS is null or oa.RFND_TS ='null',-1,DATEPART(HOUR,convert(datetime,oa.RFND_TS))))S

-------------------ROW COUNT GROUP BY----------
SELECT CHRG_NM, COUNT(*) FROM FACT_sales_order_adj_IN1542 GROUP BY CHRG_NM
SELECT  CHRG_NM,COUNT(*) FROM (
...
GROUP BY CHRG_NM
---------------------------------------STRATIFICATION -----------------------------
SELECT SUM(QTY)QTY,SUM(ADJUSMENT_AMT)ADJUSMENT_AMT FROM FACT_sales_order_adj_IN1542 
SELECT SUM(QTY)QTY,SUM(ADJUSMENT_AMT)ADJUSMENT_AMT  FROM (
...
SELECT SUM(QTY)QTY,SUM(ADJUSMENT_AMT)ADJUSMENT_AMT FROM FACT_sales_order_adj_IN1542 GROUP BY CHRG_NM
SELECT SUM(QTY)QTY,SUM(ADJUSMENT_AMT)ADJUSMENT_AMT  FROM (
...
SELECT 
--COUNT(RTN_TS_KEY	  ),
COUNT(t.ADJ_RPT_TS_KEY  )
--COUNT(charge_categ_key),
--COUNT(Rsn_cd_KEY 	  ),
--COUNT(RFND_TS_KEY	  )
FROM FACT_sales_order_adj_IN1542 oa
LEFT JOIN  
on  t.ADJ_RPT_TS_KEY =s.ADJ_RPT_TS
...
select * from [BCMPWMT].[SALES_ORDER_LINE]
...
,IIF(oa.SALES_ORDER_LINE_NUM IS NULL OR   oa. SALES_ORDER_LINE_NUM='null',101,CONVERT(INT,LTRIM(RTRIM(oa.SALES_ORDER_LINE_NUM))))as SALES_ORDER_LINE_NUM 
,IIF(oa.TENANT_ORG_ID    IS NULL OR      oa.TENANT_ORG_ID='null',101,CONVERT(INT,LTRIM(RTRIM(oa.TENANT_ORG_ID))))as TENANT_ORG_ID                                               
,IIF(oa.SHIP_NODE_ORG_ID    IS NULL OR    oa. SHIP_NODE_ORG_ID='null','N/A',LTRIM(RTRIM(oa.ship_node_org_id)))as SHIP_NODE_ORG_ID    
,IIF(oa.GIFT_IND    IS NULL OR            oa. GIFT_IND='null',101,CONVERT(int,LTRIM(RTRIM(oa.GIFT_IND))))as GIFT_IND                                                                  
,IIF(oa.ORDER_PLACED_DT  IS NULL OR       oa.ORDER_PLACED_DT='null','01-01-1900',CONVERT(DATE,LTRIM(RTRIM(oa.ORDER_PLACED_DT))))as ORDER_PLACED_DT             
,IIF(oa.ORDER_PLACED_TS  IS NULL OR       oa. ORDER_PLACED_TS='null','01-01-1900',CONVERT(DATETIME,LTRIM(RTRIM(oa.ORDER_PLACED_TS))))as ORDER_PLACED_TS                 
,IIF(oa.SVC_NM    IS NULL OR               oa.SVC_NM='null','N/A',LTRIM(RTRIM(oa.SVC_NM)))as SVC_NM          
,IIF(oa.UNIT_PRICE    IS NULL OR           oa.UNIT_PRICE='null', 0,CONVERT(float,LTRIM(RTRIM(oa.UNIT_PRICE))))as UNIT_PRICE  
,IIF(oa.UNIT_COST    IS NULL OR            oa.UNIT_COST='null',0 ,CONVERT(float,LTRIM(RTRIM(oa.UNIT_COST))))as UNIT_COST 
,IIF(oa.QTY    IS NULL OR               oa.QTY='null',0 ,CONVERT(float,LTRIM(RTRIM(oa.QTY))))as QTY 
,IIF(oa.SHPD_QTY    IS NULL OR               oa.SHPD_QTY='null','N/A',LTRIM(RTRIM(oa.SHPD_QTY)))as SHPD_QTY                 
,IIF(oa.SHIP_TO_ADDR_ID    IS NULL OR   oa.SHIP_TO_ADDR_ID='null','N/A',LTRIM(RTRIM(oa.SHIP_TO_ADDR_ID)))as SHIP_TO_ADDR_ID 
,IIF(oa.WM_ITEM_NUM    IS NULL OR         oa. WM_ITEM_NUM='null','N/A',LTRIM(RTRIM(oa.WM_ITEM_NUM)))as WM_ITEM_NUM 
,IIF(oa.UPC    IS NULL OR                oa.UPC='null','N/A',LTRIM(RTRIM(oa.UPC)))as UPC 
,IIF(oa.CATLG_ITEM_ID    IS NULL OR      oa.CATLG_ITEM_ID='null','N/A',LTRIM(RTRIM(oa.CATLG_ITEM_ID)))as CATLG_ITEM_ID   
,IIF(oa.PROD_OFFR_ID    IS NULL OR     oa. PROD_OFFR_ID='null','N/A',LTRIM(RTRIM(oa.PROD_OFFR_ID)))as PROD_OFFR_ID 
,IIF(oa.RQ_CANCEL_TS    IS NULL OR     oa. RQ_CANCEL_TS='null','01-01-1900',CONVERT(DATETIME,LTRIM(RTRIM(oa.RQ_CANCEL_TS))))as RQ_CANCEL_TS  
,IIF(oa.RQ_SHIP_TS    IS NULL OR      oa. RQ_SHIP_TS='null','01-01-1900',CONVERT(DATETIME,LTRIM(RTRIM(oa.RQ_SHIP_TS))))as RQ_SHIP_TS  
,IIF(oa.SLR_ORG_ID    IS NULL OR       oa.SLR_ORG_ID='null',101,CONVERT(INT,LTRIM(RTRIM(oa.SLR_ORG_ID))))as SLR_ORG_ID    
,IIF(oa.SHIP_TO_STORE_ORG_ID IS NULL  OR  oa.SHIP_TO_STORE_ORG_ID='null',101,CONVERT(INT,LTRIM(RTRIM(oa.SHIP_TO_STORE_ORG_ID))))as SHIP_TO_STORE_ORG_ID
,IIF(oa.ORDER_STS_ID    IS NULL OR    oa.ORDER_STS_ID='null',101,CONVERT(INT,LTRIM(RTRIM(oa.ORDER_STS_ID))))as ORDER_STS_ID   
,IIF(oa.PROD_CLASS_ID    IS NULL OR    oa.PROD_CLASS_ID='null','N/A',LTRIM(RTRIM(oa.PROD_CLASS_ID)))as PROD_CLASS_ID    
,IIF(oa.MAX_DAYS_TO_SHIP    IS NULL OR   oa.MAX_DAYS_TO_SHIP='null','N/A',LTRIM(RTRIM(oa.MAX_DAYS_TO_SHIP)))as MAX_DAYS_TO_SHIP    
,IIF(oa.EARLIEST_DLVR_TS    IS NULL OR   oa.EARLIEST_DLVR_TS='null','01-01-1900',CONVERT(DATETIME,LTRIM(RTRIM(oa.EARLIEST_DLVR_TS))))as EARLIEST_DLVR_TS   
,IIF(oa.ORIG_EXPC_DLVR_TS    IS NULL OR   oa.ORIG_EXPC_DLVR_TS='null','01-01-1900',CONVERT(DATETIME,LTRIM(RTRIM(oa.ORIG_EXPC_DLVR_TS))))as ORIG_EXPC_DLVR_TS  
,IIF(oa.CARRIER_EXPC_DLVR_TS    IS NULL OR   oa.CARRIER_EXPC_DLVR_TS='null','01-01-1900',CONVERT(DATETIME,LTRIM(RTRIM(oa.CARRIER_EXPC_DLVR_TS))))as CARRIER_EXPC_DLVR_TS 
,IIF(oa.PROMO_ID    IS NULL OR   oa.PROMO_ID='null','N/A',LTRIM(RTRIM(oa.PROMO_ID)))as PROMO_ID 
,IIF(oa.FULFMT_TYPE_ID    IS NULL OR   oa.FULFMT_TYPE_ID='null','N/A',LTRIM(RTRIM(oa.FULFMT_TYPE_ID)))as FULFMT_TYPE_ID  
,IIF(oa.ORDER_LINE_TYPE_ID    IS NULL OR   oa.ORDER_LINE_TYPE_ID='null',101,CONVERT(INT,LTRIM(RTRIM(oa.ORDER_LINE_TYPE_ID))))as ORDER_LINE_TYPE_ID 
,IIF(oa.LINE_TOT_AMT    IS NULL OR   oa.LINE_TOT_AMT='null', 0,CONVERT(float,LTRIM(RTRIM(oa.LINE_TOT_AMT))))as LINE_TOT_AMT 
,IIF(oa.ORIG_SALES_ORDER_LINE_NUM    IS NULL OR   oa.ORIG_SALES_ORDER_LINE_NUM='null',101,CONVERT(INT,LTRIM(RTRIM(oa.ORIG_SALES_ORDER_LINE_NUM))))as ORIG_SALES_ORDER_LINE_NUM 
,IIF(oa.PERISHABLE_IND    IS NULL OR   oa.PERISHABLE_IND='null',101,CONVERT(int,LTRIM(RTRIM(oa.PERISHABLE_IND))))as PERISHABLE_IND
,IIF(oa.ITEM_CLASS_ID    IS NULL OR   oa.ITEM_CLASS_ID='null',101,CONVERT(INT,LTRIM(RTRIM(oa.ITEM_CLASS_ID))))as ITEM_CLASS_ID 
,ISNULL(P.PROD_KEY,-1)     AS CATLG_ITEM_KEY 
,ISNULL(O.OFFR_KEY,-1)               AS PROD_OFFR_KEY
,isnull(DH.DAY_KEY,-1)as RQ_CANCEL_TS_KEY 
,isnull(DH1.DAY_KEY,-1)as RQ_SHIP_TS_KEY 
,ISNULL(M.ORDER_STS_MASTER_LKP_KEY,-1) AS ORDER_STS_KEY
,isnull(DH2.DAY_KEY,-1)as EARLIEST_DLVR_TS_KEY  
,isnull(DH3.DAY_KEY,-1)as ORIG_EXPC_DLVR_TS_KEY
,isnull(DH4.DAY_KEY,-1)as CARRIER_EXPC_DLVR_TS_KEY
,ISNULL(B.ORG_BUSINESS_UNIT_KEY,-1) AS SLR_ORG_KEY
,ISNULL(A.CUST_AADR1_KEY,-1) AS SHIP_TO_STORE_KEY
from
[BCMPWMT].[SALES_ORDER_LINE] oa
LEFT JOIN DIM_DAY_HOUR_sql_IN1542 DH
on  DH.DATE_ID=iif(oa.RQ_CANCEL_TS  IS NULL OR oa.RQ_CANCEL_TS ='NULL','01-01-1900',CONVERT(DATE,oa.RQ_CANCEL_TS))
and DH.hour_id=iif(oa.RQ_CANCEL_TS is null or oa.RQ_CANCEL_TS ='null',-1,DATEPART(HOUR,convert(datetime,oa.RQ_CANCEL_TS)))
left join DIM_DAY_HOUR_sql_IN1542 DH1
on  DH1.DATE_ID=iif(oa.RQ_SHIP_TS   IS NULL OR oa.RQ_SHIP_TS   ='NULL','01-01-1900',CONVERT(DATE,oa.RQ_SHIP_TS ))
and DH1.hour_id=iif(oa.RQ_SHIP_TS  is null or oa.RQ_SHIP_TS  ='null',-1,DATEPART(HOUR,convert(datetime,oa.RQ_SHIP_TS )))
left join DIM_DAY_HOUR_sql_IN1542 DH2
on  DH2.DATE_ID=iif(oa.EARLIEST_DLVR_TS  IS NULL OR oa.EARLIEST_DLVR_TS  ='NULL','01-01-1900',CONVERT(DATE,oa.EARLIEST_DLVR_TS))
and DH2.hour_id=iif(oa.EARLIEST_DLVR_TS is null or oa.EARLIEST_DLVR_TS ='null',-1,DATEPART(HOUR,convert(datetime,oa.EARLIEST_DLVR_TS)))
left join DIM_DAY_HOUR_sql_IN1542 DH3
on  DH3.DATE_ID=iif(oa.ORIG_EXPC_DLVR_TS   IS NULL OR oa.ORIG_EXPC_DLVR_TS   ='NULL','01-01-1900',CONVERT(DATE,oa.ORIG_EXPC_DLVR_TS ))
and DH3.hour_id=iif(oa.ORIG_EXPC_DLVR_TS  is null or oa.ORIG_EXPC_DLVR_TS  ='null',-1,DATEPART(HOUR,convert(datetime,oa.ORIG_EXPC_DLVR_TS )))

left join DIM_DAY_HOUR_sql_IN1542 DH4
on  DH4.DATE_ID=iif(oa. CARRIER_EXPC_DLVR_TS  IS NULL OR oa. CARRIER_EXPC_DLVR_TS   ='NULL','01-01-1900',CONVERT(DATE,oa.CARRIER_EXPC_DLVR_TS ))
and DH4.hour_id=iif(oa. CARRIER_EXPC_DLVR_TS  is null or oa. CARRIER_EXPC_DLVR_TS ='null',-1,DATEPART(HOUR,convert(datetime,oa.CARRIER_EXPC_DLVR_TS )))

LEFT JOIN IN1542.DIM_prod_SQL_IN1542 P 
ON P.CATLG_ITEM_ID=IIF(oa.CATLG_ITEM_ID    IS NULL OR      oa.CATLG_ITEM_ID='null','N/A',LTRIM(RTRIM(oa.CATLG_ITEM_ID)))
LEFT JOIN  in1542.Dim_OFFR_SQL_IN1542  O 
ON  O.OFFER_PK=IIF(oa.PROD_OFFR_ID    IS NULL OR     oa. PROD_OFFR_ID='null','N/A',LTRIM(RTRIM(oa.PROD_OFFR_ID)))

LEFT JOIN IN1542.DIM_ORDER_STS_MASTER_LKP_SQL_IN1542 M
ON M.ORDER_STS_MASTER_ID=IIF(oa.ORDER_STS_ID    IS NULL OR    oa.ORDER_STS_ID='null',101,CONVERT(INT,LTRIM(RTRIM(oa.ORDER_STS_ID)))) 

LEFT JOIN IN1542.DIM_ORG_BUSINESS_UNIT_SQL_IN1542 B ON B.ORG_ID =IIF(oa.SLR_ORG_ID    IS NULL OR       oa.SLR_ORG_ID='null',101,CONVERT(INT,LTRIM(RTRIM(oa.SLR_ORG_ID))))

LEFT JOIN IN1542.DIM_cust_addr1_sql_IN1542 A ON A.ADDR_ID=IIF(oa.SHIP_TO_STORE_ORG_ID IS NULL  OR  oa.SHIP_TO_STORE_ORG_ID='null',101,CONVERT(INT,LTRIM(RTRIM(oa.SHIP_TO_STORE_ORG_ID))))
